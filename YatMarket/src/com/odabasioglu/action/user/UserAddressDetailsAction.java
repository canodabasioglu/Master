 ...
 * @author
 */
public class UserAddressDetailsAction extends Action {
	private static final String actionRight = "UserAddressDetailsAction";
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			if (AuthorizationManager
				...
				TbAddress address = TbAddressDAO.getInstance().load(addressId);
				session.setAttribute("address", address);
				forward = mapping.findForward("updateAddressDetails");
			} else {
				request.setAttribute("message", "Error.session.expired");
				forward = mapping.findForward("sessionError");
			}
		} catch (Exception e) {
			Logger.getLogger(UserAddressDetailsAction.class).error(
