 ...
 * @author
 */
public class ProductCategoryCreateAction extends Action {
	private static final String actionRight = "ProductCategoryCreateAction";
		...
		try {
			if (AuthorizationManager
				...
				if (ImageUtility.checkFile(myFile)) {
					...
					request.setAttribute("message", "Success.category.create");
				} else {
					request.setAttribute("message", "Error.product.picture");
				}

				TbCategoryDAO.getInstance().save(category);
			...
			Logger.getLogger(ProductCategoryCreateAction.class).error(
